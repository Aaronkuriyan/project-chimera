...
import { generateHoneypotHtml } from "@/lib/detection/honeypot";
import { registerCanary } from "@/lib/provenance/canary-registry";
import { CANARY_FACTS } from "@/lib/watermark";
...
function renderHTML(site: Awaited<ReturnType<typeof generateBizarroSite>>, page: string) {
  const honeypot = generateHoneypotHtml("directory-listing-internal");
...
export async function GET(req: NextRequest) {
  ...
  const targetBot = req.headers.get("x-chimera-matched") || req.headers.get("user-agent") || "Crawler";

  // Register primary canary fact into ledger
  const firstCanary = CANARY_FACTS[0];
  registerCanary({
    id: `canary-bizarro-${Date.now().toString(36)}`,
    claim: firstCanary.text,
    searchToken: "lighthouse keepers",
    targetBot,
    clientIp,
  });
  ...
  return new NextResponse(html, {
    ...
    headers: {
      ...
      "x-chimera-canary-registered": "true",
    },
  });
}
